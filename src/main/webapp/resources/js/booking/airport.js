...
function drawDeplowestFare(dayElem , dObj){	
	//if(!safari) {		
		dayElem.find('.label').remove()
		let lowestFareData = lowestFareJson
		let departureStatin = $("#departureData").data("stationcode")
...
function exchangeRoute(){
	...
	if(selectedDepartureData.data("stationcode") != null &&  selectedArrivalData.data("stationcode") != null){
		...
		// en일 경우 공항명 css 추가 처리
		if(cultureCode.substring(0,2) == 'en'){
			if (selectedDepartureDesc[0].textContent.length > 3) {
				selectedDepartureDesc[0].textContent = selectedDepartureData.data("stationcode");
				
				let tmpDepName = selectedDepartureData.data("departurename");
				if (tmpDepName.indexOf('(') > -1) {
					tmpDepName = tmpDepName.split('(')[0]+'<br class="mobile-only">('+tmpDepName.split('(')[1];
				}
				selectedDepartureDesc.append('<em class="sub">'+tmpDepName+'</em>');
			}
		}
		...
		// en일 경우 공항명 css 추가 처리
		if(cultureCode.substring(0,2) == 'en'){
			if (selectedArrivalDesc[0].textContent.length > 3) {
				selectedArrivalDesc[0].textContent = selectedArrivalData.data("stationcode");
				
				let tmpArrName = selectedArrivalData.data("arrivalname");
				if (tmpArrName.indexOf('(') > -1) {
					tmpArrName = tmpArrName.split('(')[0]+'<br class="mobile-only">('+tmpArrName.split('(')[1];
				}
				selectedArrivalDesc.append('<em class="sub">'+tmpArrName+'</em>');
			}
		}
